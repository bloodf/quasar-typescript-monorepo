---
to: test/jest/__tests__/pages/<%= h.changeCase.camel(folder) %>/<%= h.changeCase.pascal(name) %>.spec.ts
---
import { mount, createLocalVue } from '@vue/test-utils';
import { LooseDictionary } from 'quasar/dist/types/ts-helpers';
import <%= h.changeCase.pascal(name) %> from 'pages/<%= h.changeCase.camel(folder) %>/<%= h.changeCase.camel(name) %>.vue';
